...
namespace DiscriptSimple
{
    public static class DiscriptSimple
    {
        public static string Encription(string CodeMessage, string Key)
        {
            ...
            int del = CodeMessage.Length / (CodeMessage.Length / Key.Length);

            int nDel = 0;

            while (nDel + del < CodeMessage.Length)
            {
                kodearrey.Add(CodeMessage.Substring(nDel, nDel + del));
                nDel += del;
            }
            ...
            {
                newKodeArrey.Add(kodearrey[Array.IndexOf(charKey,charKeySort[0])]);
            }
            ...
            s = encodingCh.ToArray<char>().ToString();
